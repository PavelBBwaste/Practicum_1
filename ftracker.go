...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	return ((runningCaloriesMeanSpeedMultiplier * speedMs * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH)
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	speedMs := meanSpeed(action, duration) * kmhInMsec
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(speedMs, 2)/height*cmInM)*walkingSpeedHeightMultiplier) * duration * minInH)
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	speedMs := swimmingMeanSpeed(lengthPool, countPool, duration) * kmhInMsec
	return (speedMs * +swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
}
